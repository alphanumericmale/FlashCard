...
try:
    ...
    original_data = pandas.read_csv("data/phonics.csv")
    print(original_data)
...
def next_card():
    ...
    canvas.itemconfig(card_title, text="Sound", fill="black")
    canvas.itemconfig(card_word, text=current_card["Phoneme"], fill="black")
    canvas.itemconfig(card_background, image=card_front_img)
    flip_timer = window.after(3000, func=flip_card)


def flip_card():
    canvas.itemconfig(card_title, text="Sound", fill="white")
